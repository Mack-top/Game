...
import { PlusCircle, Edit, Trash2, Code, Megaphone, ScrollText } from 'lucide-react';
...
interface GameplayElement {
  ...
  type: 'rule' | 'event' | 'script' | 'other'; // Frontend only, derived from description or fixed
  description: string; // Maps to Task.description (combined with logic)
  logic?: string; // For detailed logic/code, encoded in description
}
...
const GameplayManagement: React.FC = () => {
  ...
  const [type, setType] = useState<GameplayElement['type']>('rule'); // This will be frontend-only
  ...
  const fetchGameplayElements = async () => {
    try {
      ...
      const mappedElements: GameplayElement[] = data.map((task: any) => {
        ...
        // Attempt to derive type from logic presence or description content
  ...
  const resetForm = () => {
    ...
    setName('');
    setType('rule'); // Reset to default frontend type
    setDescription('');
    setLogic('');
  };
  ...
  const handleOpenDialog = (element?: GameplayElement) => {
    if (element) {
      ...
      setType(element.type); // Set frontend type for display
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      projectId: projectId,
      status: 'todo', // Default status for tasks
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <Card className="bg-gray-800 text-white border-gray-700 shadow-lg mb-8">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div>
            <CardTitle className="text-2xl font-bold text-blue-400">项目 {projectId} - 玩法管理</CardTitle>
